...
CSV_FILE = 'el_fuego_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_el_fuego.html')

@app.route('/api/order_fuego', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_fuego', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_fuego', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
