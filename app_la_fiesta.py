...
CSV_FILE = 'la_fiesta_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_la_fiesta_tapas.html')

@app.route('/api/order_la_fiesta', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_la_fiesta', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_la_fiesta', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
