...
CSV_FILE = 'bavarian_barrel_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_smoky_barrel.html')

@app.route('/api/order_bavarian', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_bavarian', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_bavarian', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
