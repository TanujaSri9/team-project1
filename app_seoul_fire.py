...
CSV_FILE = 'seoul_fire_bbq_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_seoul_fire.html')

@app.route('/api/order_seoul_fire', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_seoul_fire', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_seoul_fire', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
