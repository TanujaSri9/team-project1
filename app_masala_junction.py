...
CSV_FILE = 'royal_zaffran_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_masala_junction.html')

@app.route('/api/order_royal_zaffran', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_royal_zaffran', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_royal_zaffran', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
