...
CSV_FILE = 'gelato_roma_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_gelato_roma.html')

@app.route('/api/order_gelato_roma', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_gelato_roma', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_gelato_roma', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
