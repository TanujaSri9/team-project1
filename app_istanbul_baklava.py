...
CSV_FILE = 'istanbul_baklava_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_istanbul_baklava.html')

@app.route('/api/order_istanbul_baklava', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_istanbul_baklava', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_istanbul_baklava', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
