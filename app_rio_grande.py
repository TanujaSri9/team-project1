...
CSV_FILE = 'rio_grande_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_rio_grande_churrascaria.html')

@app.route('/api/order_rio_grande', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_rio_grande', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_rio_grande', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
