...
CSV_FILE = 'rio_flame_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_rio_flame_grill.html')

@app.route('/api/order_rio', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_rio', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_rio', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
