...
CSV_FILE = 'empire_burger_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_empire_burger.html')

@app.route('/api/order_empire', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_empire', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_empire', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
