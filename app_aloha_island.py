...
CSV_FILE = 'aloha_island_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_aloha_island.html')

@app.route('/api/order_aloha', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_aloha', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_aloha', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
