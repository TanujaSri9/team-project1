...
CSV_FILE = 'saigon_pho_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_saigon_pho.html')

@app.route('/api/order_saigon_pho', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_saigon_pho', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_saigon_pho', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
