...
CSV_FILE = 'lone_star_orders.csv'

@app.route('/')
def home():
    return render_template('restaurant_lone_star_smokehouse.html')

@app.route('/api/order_lone_star', methods=['POST'])
def place_order():
    data = request.json
...
@app.route('/api/orders_lone_star', methods=['GET'])
def get_orders():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
...
@app.route('/api/analytics_lone_star', methods=['GET'])
def analytics():
    if os.path.exists(CSV_FILE):
        try:
            df = pd.read_csv(CSV_FILE)
